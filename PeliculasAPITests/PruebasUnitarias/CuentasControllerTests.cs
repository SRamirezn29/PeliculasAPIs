...
namespace PeliculasAPITests.PruebasUnitarias
{
    public class CuentasControllerTests : BasePruebas
    {
        ...
        public async Task CrearUsuario()
        {
            ...
            var context2 = ConstruitContext(nombreDB);
        ...
        public async Task UsuarioPuedeLoguearse()
        {
            ...
            var controller = ConstruirCuentasController(nombreBD);
            var userInfo = new UserInfo() { Email = "ejemplo@hotmail.com", Password = "Aa123345678" };
            var respuesta = await controller.Login(userInfo);
        ...
        private CuentasCotroller ConstruirCuentasController(string nombreDB)
        {
            var context = ConstruitContext(nombreDB);
            var miUsuerStore = new UserStore<IdentityUser>(context);
            ...
            {
                {"JWT", "IYOIYIOYSFSFSODYOSSDGDGHGSASARWRETGFDDFVCXBSNSGBSA" }
            };

            var configuration = new ConfigurationBuilder()
                .AddInMemoryCollection(miConfiguracion)
                .Build();
            return new CuentasCotroller(userManager, signInManager, configuration, context, mapper);
        }
            ...
            IAuthenticationSchemeProvider schemeProvider = null) where TUser : class
        {
            ...
            var sm = new SignInManager<TUser>(manager, contextAccessor.Object, claimsFactory, options.Object,null, null, (IUserConfirmation<TUser>)schemeProvider);
